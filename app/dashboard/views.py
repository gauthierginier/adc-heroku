...
@dashboard.route('/apprenant/<apprenant>')
def apprenant(apprenant):
    apprenant = Users.query.filter_by(firstname=apprenant).one()
    print(apprenant.id)
    skills = Skills.query.filter_by(person_id=apprenant.id).all()
    ...
    return render_template('apprenant.html', apprenant=apprenant, skills_devcloud=skills_devcloud, skills_devops=skills_devops, devopspoints=devopspoints, devcloudpoints=devcloudpoints)
