...
@login_required
def profile():
    addresses = Address.query.filter_by(person_id=current_user.id).all()
    return render_template('profile.html', firstname=current_user.firstname, addresses=addresses)
